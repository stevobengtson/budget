...
// scanAccount reads the standard account column projection (no balance).
func scanAccount(s rowScanner) (Account, error) {
	var a Account
	var typ string
	var lim, apr, pay, payCat sql.NullInt64
	var archived nullTime
	if err := s.Scan(&a.ID, &a.Name, &typ, &a.StartingBalanceCents, &lim, &apr,
...
// scanAccountWithBalance reads the account projection plus the computed balance.
func scanAccountWithBalance(s rowScanner) (AccountWithBalance, error) {
	var a AccountWithBalance
	var typ string
	var lim, apr, pay, payCat sql.NullInt64
	var archived nullTime
	if err := s.Scan(&a.ID, &a.Name, &typ, &a.StartingBalanceCents, &lim, &apr,
		&pay, &a.IncludeInPaydown, &payCat, &archived, &a.CreatedAt, &a.BalanceCents); err != nil {
