...
// <div data-group> gets its own SortableJS instance whose items are the category
// rows (<div data-category>), all sharing one Sortable group name so a row can
// be dragged from one group into another. This is nested inside the list-level
// group sort (see group-sort.js); they don't collide because the category drag
// uses its own handle (.js-category-handle) and the group drag uses
// .js-group-handle.
//
...
(function () {
  function catIds(tbody) {
    return Array.prototype.map.call(
      tbody.querySelectorAll("[data-category]"),
      function (tr) {
  ...
  function initCategorySort(tbody) {
    ...
    Sortable.create(tbody, {
      draggable: "[data-category]",
      handle: ".js-category-handle",
      group: "budget-categories",
      animation: 150,
      ghostClass: "opacity-50",
      // A group's header (first child, data-group-head) isn't a draggable
  ...
  function scan(root) {
    ...
    if (root.matches && root.matches("[data-group]")) initCategorySort(root);
    root.querySelectorAll("[data-group]").forEach(initCategorySort);
  }
